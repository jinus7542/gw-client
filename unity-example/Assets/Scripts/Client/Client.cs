...
public class Client : MonoBehaviour
{
    ...
    private WsClient echoClient = null;
    ...
    void Update()
    {
        if (null != this.echoClient)
        {
            this.echoClient.DispatchMessageQueue();
        }
        if (null != this.localPlayer)
        {
    ...
    async void SendEchoMessage()
    {
        // Sending plain text
        var message = new Echo();
        message.action = "send";
        message.data = DateTime.Now.ToString();
        await this.echoClient.SendText(JsonMapper.ToJson(message));
    }

    async void ConnectToEchoServer()
    {
        InvokeRepeating("SendEchoMessage", 0.0f, 3.0f);
        this.echoClient = new WsClient();
        await this.echoClient.Connect();
    }
